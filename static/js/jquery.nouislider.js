(function ($) {
        ...
        var methods = {
            ...
            init: function init() {
                ...
                return this.each(function () {
                    ...
                    $(this).data('change', options.change);

                    $(this).find('.noUi_midBar').css({ 'left': 0, 'right': 0 });
                    ...
                    knobs.bind('mousedown.noUiSlider', function (e) {
                        ...
                        $(document).bind('mousemove.noUiSlider', function (f) {
                            ...
                            //                            var steps = new Array();
                            //                            if (settings.step) {
                            //                                var slidewidth = $('#sliderbar').css('width').replace('px', '');

                            //                                var stepper = slidewidth / settings.step;
                            //                                var substepper = stepper / 2;
                            //                                var kLeft = flattened;
                            //                                //alert(kLeft);
                            //                                for (var i = 0; i < settings.step + 1; i++) {
                            //                                    steps[i] = i * stepper;
                            //                                }
                            //                                for (var i = 0; i < settings.step; i++) {
                            //                                    if (kLeft >= steps[i] && kLeft <= steps[i + 1]) {
                            //                                        if (kLeft >= steps[i] && kLeft <= steps[i + 1] - substepper) {
                            //                                            kLeft = steps[i];
                            //                                        }
                            //                                        else {
                            //                                            kLeft = steps[i + 1];
                            ...
                            //                                    }
                            //                                }
                            //                                kLeft = kLeft <= -4 ? kLeft : (kLeft - 4);
                            //                                knob.css({ 'left': kLeft });
                            //                            }
                            //                            else {
                            //                                knob.css({ 'left': flattened });
                            //                            }
                            knob.css({ 'left': getLeft(settings.step, flattened) });
                    ...
                    $(this).bind('click.noUiSlider', function (e) {
                        ...
                        if ($(this).data('activated')[0] && $(this).data('activated')[1]) {
                            ...
                            if (dot0 > z) {
                                ...
                                $(this).children(".noUi_upperHandle").css("left", getLeft(settings.step, (dot0 - thebar)));
                            } else {
                                //$(this).children(".noUi_lowerHandle").css("left", (dot0 - thebar));
                                $(this).children(".noUi_lowerHandle").css("left", getLeft(settings.step, (dot0 - thebar)));
                            }
            ...
            move: function move() {
                ...
                if (settings.point == 'lower' || settings.point == 0 || ok) {
                    ...
                    if (settings.moveStyle == 'animate') {
                        ...
                        newKnob1.animate({ 'left': getLeft(settings.step, value1) }, { step: function () { if (settings.bar && settings.bar != 'off') { rebuildMidBar(element); } } });
                    } else {
                        //newKnob1.css('left', value1);
                        newKnob1.css('left', getLeft(settings.step, value1));
                    }
                }
    ...
    function getLeft(s, l) {
        var steps = new Array();
        if (s) {
            var slidewidth = $('#sliderbar').css('width').replace('px', '');

            var stepper = slidewidth / s;
            var substepper = stepper / 2;
            var kLeft = l;
            //alert(kLeft);
            for (var i = 0; i < s + 1; i++) {
            ...
            kLeft = kLeft <= -4 ? kLeft : (kLeft - 4);
            kLeft = kLeft >= steps[steps.length - 1] - 4 ? (steps[steps.length - 1] - 4) : kLeft;
            //alert(kLeft);
